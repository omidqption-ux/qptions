...
export default function UserMetaCard() {
  ...
  const { username, fullName, role } = useSelector((store: RootState) => store.admin)
  const { isOpen, openModal, closeModal } = useModal();
  const [value, setValue] = useState(fullName)
  const handleSave: (e: any) => void = async (e: any) => {
    e.preventDefault()
    try {
      const res = await axiosInstance.put('/admins/changeName', { fullName: value })
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[700px] m-4">
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          ...
          <form className="flex flex-col">
            <div className="custom-scrollbar h-[140px] overflow-y-auto px-2 pb-3">
              <div className="mt-7">
            ...
            <div className="flex items-center gap-3 px-2 mt-6 lg:justify-end">
              ...
              <Button variant="contained" onClick={(e: any) => handleSave(e)}>
                Save Changes
              </Button>
            </div>
          </form>
