...
import { setRejectedVerificationList, Verification } from "../../store/slices/verification"
...
const Approved = ({ setLoading }: { setLoading: Dispatch<SetStateAction<boolean>> }) => {
    ...
    const getList = async () => {
        try {
            setLoading(true)
            const res = await axiosInstance.get('/verifications/rejected', {
                params: {
                    page,
                    limit,
                    sortBy: 'createdAt',
            ...
            dispatch(setRejectedVerificationList(res.data.data.map((verification: Verification) => ({
    ...
    return (
        <div className="w-full rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
            <RejectedModal isOpen={isOpen} closeModal={closeModal} selectedVerificationId={selectedVerificationId} />
            <div className="w-full overflow-x-auto">
                <Table  >
                    ...
                    <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                        {verificationRejectedList?.map((verification: Verification) => (
