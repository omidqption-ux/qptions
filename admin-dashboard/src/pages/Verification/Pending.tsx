...
import { setPendingVerificationList, Verification } from "../../store/slices/verification"
...
const Pending = ({ setLoading }: { setLoading: Dispatch<SetStateAction<boolean>> }) => {
    ...
    const { verificationPendingList } = useSelector((store: RootState) => store.verification)
    const getList = async () => {
        try {
            setLoading(true)
            const res = await axiosInstance.get('/verifications/pending', {
                params: {
                    page,
                    limit,
                    sortBy: 'createdAt',
            ...
            dispatch(setPendingVerificationList(res.data.data.map((verification: Verification) => ({
    ...
    return (
        <div className="w-full rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
            <PendingModal isOpen={isOpen} closeModal={closeModal} getList={getList} selectedVerificationId={selectedVerificationId} />
            <div className="w-full overflow-x-auto">
                <Table  >
                    ...
                    <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                        {verificationPendingList?.map((verification: Verification) => (
