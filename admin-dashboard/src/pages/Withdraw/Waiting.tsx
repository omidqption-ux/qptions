...
import { setPendingWithList, Withdraw } from "../../store/slices/withdraw"
import PreviewIcon from '@mui/icons-material/Preview';
import WaitingModal from "./WaitingModal"
import { useModal } from "../../hooks/useModal"
import { normalizeDate } from "../../utils/normalizedate"

const Pending = ({ setLoading, getCounts }: { setLoading: Dispatch<SetStateAction<boolean>>, getCounts: () => Promise<void> }) => {
    ...
    const { pendingList } = useSelector((store: RootState) => store.withdraw)
    const getList = async () => {
        try {
            setLoading(true)
            ...
            dispatch(setPendingWithList(res.data.data.map((witdraw: Withdraw) => ({
                _id: witdraw._id,
    ...
    return (
        <div className="w-full rounded-xl border border-gray-200 bg-white dark:text-gray-300 dark:border-white/[0.05] dark:bg-white/[0.03]">
            <WaitingModal isOpen={isOpen} closeModal={closeWithModal} selectedWithdrawId={selectedWithdrawId} isWaiting={true} />
            <div className="w-full overflow-x-auto">
                <Table  >
                    ...
                    <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                        {pendingList?.map((withdraw: Withdraw) => (
