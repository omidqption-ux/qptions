...
import { useRecaptchaV2, runV3 } from '@/utils/useRecaptcha'
...
export default function Registeration({ open, handleClose, isRegister }) {
     ...
     // --- FACTORY SCHEMAS (so t() is available) ---
     const phoneRegexOptional = /^$|\+[1-9]\d{7,14}$/; // allow empty OR +XXXXXXXX (8–15 digits)

     // Helper to trim values before validation
     ...
     const makeRegisterSchema = () =>
          Yup.object().shape({
               ...
               password: Yup.string()
                    .matches(/^(?=.*[A-Z])(?=.*\d).{8,}$/, t('errors.passwordPattern'))
               ...
               email: Yup.string()
                    ...
                    .when('isPhone', {
                         is: 'email',
                         then: (s) => s.required(t('errors.emailRequired')),
                         otherwise: (s) => s.notRequired(),
                    }),

               phone: Yup.string()
                    .transform(trimValue)
                    .matches(phoneRegexOptional, t('errors.phoneInvalid'))
                    .when('isPhone', {
                         is: 'phone',
                         then: (s) =>
                              s
     ...
     const { containerRef, run: runV2 } = useRecaptchaV2()
     const [msg, setMsg] = React.useState(null)
     const [recapchaLoading, setrecapchaLoading] = React.useState(false)
     ...
     // --- API actions (messages localized) ---
     const registerUser = async (values) => {
          try {
               setMsg(null)
               setrecapchaLoading(true)
               setLoading(true)
               let tokenV3 = ''
               try { tokenV3 = await runV3('register') } catch { }
               setServerError('')
               const res = await axios.post('/api/auth/register', {
                    ...
                    recaptcha: tokenV3 ? { version: 'v3', token: tokenV3, action: 'register' } : { version: 'v3', token: '', action: 'register' }
               })
               if (res.ok) {
                    window.location.replace(process.env.NODE_ENV !== 'development' ? 'https://panel.qption.com/TradingRoom' : 'http://localhost:3001/TradingRoom')
                    return
               ...
               if (res?.needV2) {
                    await runV2(
                         async (tokenV2) => {
                              const res2 = await axios.post('/api/auth/register', {
                                   phone: values.phone,
                                   email: values.email,
                                   password: values.password,
                                   country: values.country,
                                   recaptcha: { version: 'v2', token: tokenV2 }
                              })
                              if (res2.ok) {
                                   window.location.replace(process.env.NODE_ENV !== 'development' ? 'https://panel.qption.com/TradingRoom' : 'http://localhost:3001/TradingRoom')
                              } else {
                                   setMsg(res2?.message || t('errors.recaptchaV2Failed'))
                              }
                         },
                         (err) => setMsg(err)
                    )
     ...
     const loginUser = async (values) => {
          try {
               ...
               setLoading(true)
               let tokenV3 = ''
               try { tokenV3 = await runV3('login') } catch { }
               setServerError('')
               const res = await axios.post('/api/auth/login', {
                    phone: values.phone,
                    email: values.email,
                    password: values.password,
                    recaptcha: tokenV3 ? { version: 'v3', token: tokenV3, action: 'login' } : { version: 'v3', token: '', action: 'login' }
               })
               if (res.ok) {
                    window.location.replace(process.env.NODE_ENV !== 'development' ? 'https://panel.qption.com/TradingRoom' : 'http://localhost:3001/TradingRoom')
                    return
     ...
     const sendOtpLogin = async (values) => {
          try {
               ...
               if (values.isPhone === 'phone') {
                    ...
                    await axios.post('/api/auth/sendOtpSMS', { phone: values.phone })
                    setEnteredPhone(values.phone)
               } else {
                    await axios.post('/api/auth/sendOtpEmail', { email: values.email })
     ...
     return (
          <Modal
               open={open && !isLogin}
               onClose={closeModalPanel}
               keepMounted
               disableAutoFocus
               aria-labelledby={t('aria.modalTitle')}
               aria-describedby={t('aria.modalDesc')}
               className='z-10'
          >
               <main className="flex flex-col justify-around items-center min-h-screen backdrop-blur-lg w-full relative overflow-visible p-12" dir={isRTL ? 'rtl' : 'ltr'}>
                    <Box className="relative my-5 flex flex-col justify-center bg-linear-to-br from-green-500/10 to-green-500/5 backdrop-blur-3xl rounded-xl p-3 md:p-4 border border-green-500/20">
                         <div className="flex w-full justify-end">
                              <CloseIcon
                                   fontSize="small"
                                   onClick={handleClose}
                                   className="text-white hover:text-green-400 cursor-pointer hover:rotate-90 transition-all duration-300"
                              />
                         </div>
                         <Box className="w-full text-OffWhite">
                              ...
                              <CustomTabPanel value={tabValue} index={0}>
                                   <Formik
                                        enableReinitialize
                                        initialValues={{ phone: '', email: '', password: '', termsAccepted: false, isPhone: 'email', country: '' }}
                                        ...
                                        {(props) => (
                                             <form autoComplete="off" className="flex h-[200px] w-full flex-col gap-2" onSubmit={props.handleSubmit}>
                                                  <div className="flex flex-col justify-start w-full">
                                                       ...
                                                       {props.touched.phone && props.errors.phone && <span className="mx-auto max-w-[280px] text-xs text-red-500 text-center wrap-break-word">{props.errors.phone}</span>}
                                                       {props.touched.email && props.errors.email && <span className="mx-auto max-w-[280px] text-xs text-red-500 text-center wrap-break-word">{props.errors.email}</span>}
                                                  </div>
                                                  ...
                                                  <div className="flex flex-col justify-start">
                                                       ...
                                                       {props.touched.password && props.errors.password &&
                                                            <div
                                                                 className="mx-auto max-w-[280px] text-xs text-red-500 text-center wrap-break-word"
                                                            >
                                                  ...
                                                  <div className="flex flex-col justify-start">
                                                       ...
                                                       {props.submitCount > 0 && props.errors.termsAccepted && <span className="mx-auto max-w-[280px] text-xs text-red-500 text-center wrap-break-word">{props.errors.termsAccepted}</span>}
                                                  </div>
                                                  <Button
                                                  ...
                                                  >
                                                       {loading ? <Loading size={45} /> : t('buttons.register')}
                                                  </Button>
                              ...
                              <CustomTabPanel value={tabValue} index={1}>
                                   <Formik
                                        enableReinitialize
                                        initialValues={{ phone: '', email: '', password: '', isPhone: 'email' }}
                                                  ...
                                                  <div className="w- flex h-[200px] flex-col gap-4 w-full">
                                                       ...
                                                       <div className="flex w-full flex-col items-center justify-between">
                                                            <Button
                                                                 ...
                                                                 aria-busy={loading || undefined}
                                                            >
                                                                 {loading ? <Loading size={45} /> : t('buttons.login')}
                                                            </Button>
                              ...
                              <CustomTabPanel value={tabValue} index={2}>
                                   {otpWasSent && !otpExpired ? (
                                                            ...
                                                            {(props) => (
                                                                 <form autoComplete="off" onSubmit={props.handleSubmit} className="flex  items-center justify-center h-[34px] w-full gap-2">
                                                                      <input
                                                                           ...
                                                                           id="otpInput"
                                                                           type="text"
                                                                           className="placeholder-gray-400 text-white mx-1 h-[38px] rounded-md bg-[#20293E] p-3 placeholder:text-xs focus:outline-none w-[120px]"
                                        ...
                                        <div className="flex h-[200px] w-full flex-col justify-center gap-4">
                                             <Formik
                                                  enableReinitialize
                                                  initialValues={{ phone: '', email: '', isPhone: 'email' }}
