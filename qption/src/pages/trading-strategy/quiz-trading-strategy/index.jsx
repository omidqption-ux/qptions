...
export default function TradingStrategiesQuizPage() {
    const { t: tq, i18n } = useTranslation('quiz-trading-strategy')
    const isRTL = ['fa', 'ar', 'ur', 'he'].includes(i18n.language)

    // Pull question pool from translations (same pattern as your reference file)
    const pool = useMemo(() => {
    ...
    // Loading state (same SEO/meta approach)
    if (!pool.length) {
        return (
            <>
                <Head>
    ...
    return (
        <>
            ...
            <main
                dir={isRTL ? 'rtl' : 'ltr'}
                className="bg-linear-to-b from-[#142B47] to-[#142B47]/90 w-full flex flex-col items-center justify-start p-4 lg:p-12 min-h-screen"
            >
                {/* Hero */}
                <motion.div
                    ...
                    className="flex w-full text-center justify-center items-center py-16"
                >
                ...
                <motion.div
                    ...
                    whileInView={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.8 }}
                    className="w-full max-w-3xl mx-auto p-6 bg-[#0a1a2e] rounded-2xl shadow-2xl border border-white/10"
                >
                    {/* Header */}
                    ...
                    {!finished ? (
                            ...
                            <div className="space-y-3">
                                {currentQ?.options?.map((opt, idx) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <span className="font-medium w-full">{opt}</span>
                                        </button>
                                    )
                                })}
                            </div>
                                ...
                                {locked ? (
                                    ...
                                    >
                                        <p className="font-semibold">
                                            {selected === currentQ.answer ? tq('quiz.correct') : tq('quiz.incorrect')}
                                        </p>
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-blue-100 mt-2">
                                {score >= 8 ? tq('results.tier.excellent') : score >= 5 ? tq('results.tier.good') : tq('results.tier.retry')}
                            </p>
...
export async function getServerSideProps({ locale }) {
    return {
        props: {
            ...(await serverSideTranslations(locale, ['common', 'nav', 'footer', 'auth', 'quiz-trading-strategy'], i18nConfig)),
        },
    }
}