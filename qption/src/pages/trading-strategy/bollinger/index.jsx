...
export default function Bollinger() {
     const { t, i18n } = useTranslation('bollinger')
                    ...
                    >
                         <div className="w-full max-w-7xl px-4">
                              ...
                              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
                                   <Link href="/trading-strategy" className="text-green-500 hover:text-green-400 transition-colors duration-300 flex items-center gap-2">
                                        <FaChartLine className="text-lg my-6" />
                                        <span className="text-lg">{t('breadcrumb.tradingStrategy')}</span>
                                   </Link>
                              </motion.div>
                              ...
                              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-8">
                                   <div className="bg-linear-to-br from-green-500/10 to-green-500/5 backdrop-blur-3xl rounded-xl p-3 md:p-4 border border-green-500/20">
                                        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                                             ...
                                             <Button onClick={handleOpen} className="normal-case bg-green-500 hover:bg-green-600 text-white transition-colors duration-300" variant="contained" size="medium">
                              ...
                              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="text-center mb-12">
                                   ...
                                   <p className="text-green-100 text-lg max-w-7xl mx-auto text-center">{hero?.subtitle}</p>
                              </motion.div>

                              {/* Video */}
                              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="w-full my-5 relative bg-linear-to-br from-green-500/10 to-green-500/5 backdrop-blur-3xl rounded-xl p-1 md:p-4 border border-green-500/20 flex flex-col">
                                   <video
                                        src="/videos/boilingerBand/boilinger.mp4"
                                        poster="/videos/boilingerBand/boilinger-poster.png"
                                        controls
                                        playsInline
                                        preload="metadata"
                              ...
                              <div className="space-y-8 my-4">
                                   ...
                                   <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="bg-linear-to-br from-green-500/10 to-green-500/5 backdrop-blur-3xl rounded-xl p-3 md:p-4 border border-green-500/20">
                                        <div className="flex items-center gap-3 mb-4">
                                             ...
                                             <h2 className="text-2xl font-bold text-green-500">{t('strategies.title')}</h2>
                                        </div>
...
export async function getServerSideProps({ locale }) {
     return {
          props: {
               ...(await serverSideTranslations(locale, ['common', 'bollinger', 'nav', 'footer', 'auth'], i18nConfig)),
          },
     }
}
