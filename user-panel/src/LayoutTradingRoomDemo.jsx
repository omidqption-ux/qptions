...
import NavbarDemo from './pages/TradingRoomDemo/Components/NavbarDemo'
import CurrencyDisplay from './components/NumberFormat/NumberFormat'
import SyncIcon from '@mui/icons-material/Sync'
import { setDemoBalance, setUserSettings } from './redux/slices/userSlice'
...
export default function MiniDrawer({ socket }) {
     ...
     const getUserTradingRoomInfo = async () => {
          try {
               const tradingRoomRes = await axiosInstance.post(
                    '/tradingRoom/setTradingRoomDemo'
               )
     ...
     const refreshDemoBalance = useCallback(async () => {
          try {
               ...
               if (userSettings.soundControl.notification) {
                    playWin()
               }
               setRotate(true)
               ...
               // 5) stop rotation after 0.5s
               setTimeout(() => setRotate(false), 500)
          } catch (e) {
          } finally {
               setBalanceLoading(false)
          }
     }, [dispatch, userSettings.soundControl.notification])
     ...
     // useEffect(() => {
     //      /* 1 ─ Add a CSS rule BEFORE LiveChat loads.
     //             Because it’s injected first, the element is hidden the instant it
     //             enters the DOM, so nothing flashes on screen.                  */
     ...
     //      /* 3 ─ OPTIONAL extra guard: if LiveChat later rewrites inline styles,
     //             a MutationObserver forces the element back to `display:none`.
     //             (Not usually needed, but costs ~0 KB.)                          */
     ...
     const openWhenReady = () => {
          // LC_API can exist but still be half‑baked; check the method itself
          if (
               window.LC_API &&
               ...
               // retry in 150 ms until it’s available
     ...
     return (
          <Box className='select-none relative flex bg-darkStart'>
               <CssBaseline />
               <AppBar className={`z-0 bg-slate-900 flex justify-between`}>
                    <Toolbar
  ...
  [&_.MuiDrawer-paper]:border-LightNavy absolute lg:relative z-50
                       ${open
               ...
               >
                    ...
                    <List className='bg-darkStart h-full text-menuTxt flex flex-col justify-between'>
                                        ...
                                        {open ? (
                                             <div className='flex items-center'>
                                                  <SyncIcon
                                                       ...
                                                       className={`transition-all duration-300  ${rotate
                                                            ? 'rotate'
                                                            : ''
                                                            }`}
                                                  />
