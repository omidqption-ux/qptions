...
const Payout = () => {
     ...
     const { activeTickerDemo } = useSelector((store) => store.ticker)
     const { payOutPercentage, timer } = useSelector((store) => store.trade)
     useEffect(() => {
          const next = Number(activeTickerDemo?.payoutPercentage ?? 0);
          ...
          if (timer > 599) {
               dispatch(setPayoutPercentage({
                    payOutPercentage:
                         activeTickerDemo.payoutPercentage - 6
               }))
          }
     }, [activeTickerDemo, timer]);
     return (
