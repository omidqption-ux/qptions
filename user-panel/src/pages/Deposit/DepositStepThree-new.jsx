...
const DepositStepThree = ({ setActiveStep }) => {
    ...
    const generateQRCode = () => {
        setQrData(`${paymentInfo.pay_address}`)
    }

    const fetchAddress = useCallback(async () => {
        try {
            setAddrLoading(true)
            const res = await axiosInstance.get(`/api/deposits/address?chain=${paymentInfo.chain.toLowerCase()}`)
    ...
    const networkLabel = paymentInfo.chain === 'ETH' ? 'USDT – ERC20 (Ethereum)' : 'USDT – TRC20 (Tron)'
